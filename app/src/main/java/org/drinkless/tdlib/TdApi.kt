...
object TdApi {
    ...
    // Data Classes (defined first to avoid forward-reference issues)
    class Ok : Object()
    ...
    class File(val id: Int = 0, val size: Long = 0L, val local: LocalFile = LocalFile(), val remote: RemoteFile = RemoteFile()) : Object()
    class User(
        val id: Long = 0L,
    ...
    class PhoneNumberAuthenticationSettings(
        ...
        var isCurrentPhoneNumber: Boolean = false,
        var hasUnknownPhoneNumber: Boolean = false,
        var allowSmsRetrieverApi: Boolean = false
    )
